...
##   * the PLACED station satisfies the craft station-gate: Station.tags_in_range finds its &"forge" tag and a
...
## -90000, elevation 48000, pebble -60000, ...), so no stray station wanders into these tags_in_range scans and
...
func run(ctx: TestContext) -> void:
	...
	# master_cordage (station_tag &"forge") + 5 fiber. Beside the PLACED station the scan returns [&"forge"] and
	# the craft succeeds (fiber 5 -> 0, cord 0 -> 3) -- the tags flowing placed-Station -> tags_in_range ->
	...
	var near_tags: Array[StringName] = Station.tags_in_range(HOME, Station.DEFAULT_REACH)
	var inv_near: Inventory = Inventory.new()
	inv_near.add_item(FIBER, 5)
	var near_ok: bool = craft.craft(MASTER, sheet, inv_near, near_tags)
	ctx.check(learned and near_tags.has(&"forge") and near_ok
			and inv_near.count_of(FIBER) == 0 and inv_near.count_of(CORD) == 3,
		"the PLACED station satisfies the station-gate: tags_in_range finds &\"forge\" and master_cordage crafts beside it (fiber 5 -> 0, cord 0 -> 3)",
		"placed station did not satisfy the craft gate (learned=%s, tags=%s, ok=%s, fiber=%d, cord=%d)" % [str(learned), str(near_tags), str(near_ok), inv_near.count_of(FIBER), inv_near.count_of(CORD)])

	var away: Vector2 = HOME + Vector2(0.0, 6000.0)
	var away_tags: Array[StringName] = Station.tags_in_range(away, Station.DEFAULT_REACH)
	var inv_away: Inventory = Inventory.new()
	inv_away.add_item(FIBER, 5)
	var away_ok: bool = craft.craft(MASTER, sheet, inv_away, away_tags)
	ctx.check(away_tags.is_empty() and not away_ok and inv_away.count_of(FIBER) == 5,
		"far from the placed station the gate closes: tags_in_range [] -> master_cordage refuses, fiber stays 5",
		"placed-station gate leaked far away (tags=%s, ok=%s, fiber=%d)" % [str(away_tags), str(away_ok), inv_away.count_of(FIBER)])
